...
func staticHandler(next HandlerFunc) HandlerFunc {
	...
	return func(c *Context) {
		// HTTP Method 가 GET 이나 HEAD 가 아닌 경우, 바로 다음 핸들러 수행 ?
		...
		f, err := dir.Open(file)
		if err != nil {
			// URL 경로에 해당하는 파일 open 이 실패하면, 바로 다음 핸들러 수행
			next(c)
			return
		}
		defer f.Close()
		...
		// URL 경로가 디렉토리인 경우 indexFile 을 사용
		if fi.IsDir() {
			// 디렉토리 경로를 URL 로 사용하는 경우
			// 경로의 마지막에 /를 붙여야 한다
			...
			// 디렉토리를 가리키는 URL 경로에 indexFile 명을 붙여 전체 파일 경로 생성
		...
		// file 의 내용 전달
		// next 핸들러로 제어권을 넘기지 않고 요청 처리를 종료함
